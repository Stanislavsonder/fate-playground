import {Armor as IArmor, ArmorModifier, CharacterSize, ArmorSlot, ArmorType} from '../types'
import {
...
export class Armor implements IArmor {
    ...
    get baseDefence(): number {
        return this.defence * (this.getTypeModifier().defenceMultiplier +
            (this.bonuses?.defenceMultiplier || 0) +
            (this.penalty?.defenceMultiplier || 0))
    }
    ...
    get evadeChance(): number {
        return this.getTypeModifier().evadeChance + (this.bonuses?.evadeChance || 0) + (this.penalty?.evadeChance || 0)
    }
}
